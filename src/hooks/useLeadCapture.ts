...
/**
 * Enregistre un prospect dans la base du site (en plus de GoHighLevel).
 * Retourne les coordonnees retenues, ou null si la soumission est un doublon recent.
 */
export const saveLead = async (payload: AnyRecord): Promise<LeadContact | null> => {
  const flat = flatten(payload);
  const fingerprint = JSON.stringify({ flat, t: Math.floor(Date.now() / 60000) });
  if (sessionStorage.getItem(STORAGE_KEY) === fingerprint) return null;
  sessionStorage.setItem(STORAGE_KEY, fingerprint);

  const composedName = [
    pick(flat, ["first_name", "firstname", "prenom"]),
    pick(flat, ["last_name", "lastname", "nom_famille"]),
  ]
    .filter(Boolean)
    .join(" ");
  ...
  try {
    localStorage.setItem("lgm_lead_contact", JSON.stringify(contact));
  } catch {
